...
key_dict = {'contributor': lambda key, l: parse_item_list(key, l, allow_not_lang=True),
            'title': lambda key, l: parse_item_list(key, l, allow_lang=True,
                                           allow_not_lang=True),
            'creator': lambda key, l: parse_item_list(key, l, allow_not_lang=True),
            'subject': lambda key, l: parse_item_list(key, l, allow_lang=True),
            'source': lambda key, l: parse_item_list(key, l, allow_lang=True),
            'description': lambda key, l: parse_item_list(key, l, allow_lang=True),
            'publisher': lambda key, l: parse_item_list(key, l, allow_lang=True),
            'date': lambda key, l: parse_item_list(key, l, allow_not_lang=True),
            'type': lambda key, l: parse_item_list(key, l, allow_lang=True),
            'format': lambda key, l: parse_item_list(key, l, allow_lang=True,
                                            allow_not_lang=True),
            'identifier': lambda key, l: parse_item_list(key, l, allow_lang=True,
                                                allow_not_lang=True),
            'coverage': lambda key, l: parse_item_list(key, l, allow_none=True, allow_lang=True,
                                              allow_not_lang=True),
            'language': lambda key, l: parse_item_list(key, l, allow_not_lang=True),
            'rights': lambda key, l: parse_item_list(key, l, allow_lang=True)
            }
...
def convert_xml_data_to_dataframe(data_dict: dict, ITEM_PREFIX='{https://geoscan.nrcan.gc.ca/schema/osdp_feed/1.0/}', SUB_KEY_PREFIX='{http://purl.org/dc/elements/1.1/}'):
    """
    ...
    """
    prefixed_key_dict = {SUB_KEY_PREFIX + k:v for k,v in key_dict.items()}
    df_list = []
    for i, item in enumerate(data_dict[ITEM_PREFIX + 'item']):
        item_dict = {}
        for key, value in item.items():

            ret_dict = prefixed_key_dict[key](key, value)
...
def convert_xml_to_dataframe_and_save(xml_schema: str, xml_file: str, validate: bool, ITEM_PREFIX=None, SUB_KEY_PREFIX=None):
    """
    ...
    xml_df_file = f'{xml_file}_df.parquet'
    if Path(xml_df_file).exists():
        print('Loading existing df file...')
        return pd.read_parquet(xml_df_file)
    ...
    df = convert_xml_data_to_dataframe(data_dict, ITEM_PREFIX=ITEM_PREFIX, SUB_KEY_PREFIX=SUB_KEY_PREFIX)
    print(df.head())
    df.to_parquet(xml_df_file)

    return df


if __name__ == "__main__":
    schema_file = 'data/nrcan/geoscan.xsd'
