...
""" Compile results from bert keyword evaluation 
    ...
    OUTPUT_KERAS_RUN_LOG and OUTPUT_KERAS_EVAL_METRICS which are 
...
input_dir = pathlib.Path('/nrcan_p2/data/07_model_output/keyword_prediction_keras/')

OUTPUT_KERAS_RUN_LOG = 'keras_runs_log.csv'
OUTPUT_KERAS_EVAL_METRICS = 'keras_eval_data.parquet'
...
for run_dir in run_dirs: 
    ...
    if not input_log_file.exists():
        ...
        for i_set in i_sets:
            ...
            model_file = split_dir / "model.keras"
            model_history_file = split_dir / "model_history.json" 
            if not metrics_file.exists() or not model_file.exists() or not model_history_file.exists():
                is_illegal = True
                break
    ...
    if is_illegal:
        ...
        if input_log_file.exists():
            with open(input_log_file, 'r') as f:
                input_log = json.load(f)

            illegal_input_info.append(input_log)
        else:
            illegal_input_info.append({})
...
illegal_input_info = pd.DataFrame(illegal_input_info)

concat_info = pd.concat([legal_runs, input_info], axis=1)
concat_info2 = pd.concat([illegal_or_unfinished_runs, illegal_input_info], axis=1)
concat_info = pd.concat([concat_info, concat_info2], axis=0)
...
concat_info.to_csv(OUTPUT_KERAS_RUN_LOG)

eval_datas = pd.concat(eval_datas)
print(len(eval_datas))
eval_datas.to_parquet(OUTPUT_KERAS_EVAL_METRICS)
