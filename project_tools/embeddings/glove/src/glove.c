...
//     char name[40];
//     tmpnam(name);
//     FILE *debug_file= fopen(name, "wb");
//     if (debug_file == NULL) {
//         log_file_loading_error("debug file", name);
//         pthread_exit(NULL);
//     }
//     printf("Writing to debug file: %s\n", name);
    ...
    for (a = 0; a < lines_per_thread[id]; a++) {
        ...
        // debug
//        if (cr.word2 >= vocab_size) {
//            printf("%d", cr.word2);
//            printf("%lld", vocab_size);
//        }
//
//        if (cr.word1 >= vocab_size) {
//            printf("%d", cr.word1);
//            printf("%lld", vocab_size);
//        }

//         fwrite(&cr, sizeof(CREC), 1, debug_file);

        long long W_size = 2 * vocab_size * (vector_size + 1);
        if (!(vector_size + l1 > 0 && vector_size + l1 < W_size)) {
            printf("%lld\n", vocab_size);
            printf("%d\n", vector_size);
            printf("%lld\n", l1);
            printf("%lld\n", W_size);
